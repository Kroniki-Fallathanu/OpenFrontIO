...
export async function startWorker() {
  ...
  // Hexah fork: with the external API switched off there is no matchmaking
  // service to check in with, so the ranked loop never starts.
  if (!ServerEnv.externalApiDisabled()) {
    setTimeout(
      () => {
  ...
  if (!ServerEnv.externalApiDisabled()) {
    // PrivilegeChecker falls back to fail-open defaults when never started.
  ...
  // Hexah „Dzikie Ziemie": a trusted server-to-server caller creates a battle
  // under an id it picked itself (the battle id), authorized by a shared key
  // instead of a player JWT — that deployment runs with the external API off,
  // so player tokens do not exist. No key configured = route disabled.
  app.post("/api/create_game/:id", async (req, res) => {
    ...
    const creatorPersistentID =
      typeof pid === "string" && pid.length > 0 ? pid : undefined;
    ...
    // The caller derives the worker from the id with the same hash we do, so a
    // mismatch means the request landed on the wrong worker.
