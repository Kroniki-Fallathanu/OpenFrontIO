...
export class GameServer {
  ...
  private archiveGame() {
    ...
    this.deps.archive(partialRecord);
    // Hexah „Dzikie Ziemie": notify Strapi of the result (S2S, HMAC; no-op
    // unless RESULT_WEBHOOK_* env is set). The finalized record is kept so the
    // same result can be published again when it never reached the host.
    const gameRecord = finalizeGameRecord(partialRecord);
  ...
  private handleWinner(client: Client, clientMsg: ClientSendWinnerMessage) {
    ...
    if (this.winnerVote.winner() !== null || client.reportedWinner !== null) {
