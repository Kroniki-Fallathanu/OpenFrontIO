...
export class ClientGameRunner {
  ...
  public start() {
    ...
    this.worker.start((gu: GameUpdateViewData | ErrorUpdate) => {
      ...
      // `updates` is absent on packed-only frames, so the win is read
      // defensively — this hook must never break the render loop.
      const winUpdate = gu.updates?.[GameUpdateType.Win]?.[0];
      if (winUpdate !== undefined) {
        this.hexahVictoryCheck?.rememberWin(winUpdate);
      }
    });
